...
export default function Beams() {
  ...
  const [beams_mask] = useTexture(["/textures/beams_mask.png"]);
  ...
  return (
    <>
      {beamsGeometries.map((geometry, index) => (
        <Beam
          geometry={geometry}
...
function Beam({ geometry, beams_mask, beam_index }) {
  const { vectors, initialPositionAttribute } = useMemo(() => {
    ...
    vectors.sort((a, b) => a.y - b.y); // sorts an array of vectors in ascending order based on their y-coordinate values
  ...
  useFrame((state) => {
    ...
    transformVector.applyAxisAngle(new Vector3(0, 1, 0), elapsed * 0.25 + beam_index * 17.87975);
    ...
    for (let i = 0; i < currentPositionAttribute.count; i++) {
      ...
      const isTopVertex =
        (vector.x == vectors[2].x && vector.y == vectors[2].y && vector.z == vectors[2].z) ||
        (vector.x == vectors[3].x && vector.y == vectors[3].y && vector.z == vectors[3].z);
